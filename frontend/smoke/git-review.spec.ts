...
test("popout review keeps commit navigation after patch rerenders", async ({ page }, info) => {
  ...
  try {
    ...
    await panel(page).getByRole("button", { name: "History", exact: true }).click();
    await expect(panel(page).locator(".git-selected-commit-meta")).toContainText(a.head);
    ...
    popout = await opened;
    await expect(panel(popout).locator(".git-selected-commit-meta")).toContainText(a.head);
    ...
    await expect(panel(popout).locator(".git-selected-commit-meta")).toContainText(a.initial);
...
test("commit shortcuts stay in the focused review and survive asynchronous rerenders", async ({ page }) => {
  ...
  try {
    ...
    const view = panel(page);
    await view.getByRole("button", { name: "History", exact: true }).click();
    await expect(view.locator(".git-selected-commit-meta")).toContainText(a.head);
    await view.locator(".git-history-heading strong").click();
    await page.keyboard.press("p");
    await expect(view.locator(".git-selected-commit-meta")).toContainText(a.commits[2]);
    await expect(view.locator(".diff-line-add")).toContainText("keyboard_2");
    await page.keyboard.press("p");
    await expect(view.locator(".git-selected-commit-meta")).toContainText(a.commits[1]);
